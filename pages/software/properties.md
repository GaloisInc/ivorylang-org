...
Because the C code is generated, the code might be about half that size if
hand-written.  Nonetheless, SMACCMPilot (or any fully-featured autopilot) will
be 10s of thousands or more of lines of code.
...
than analyzing existing artifacts for bugs, so we want to go better than the
"bug-hunting" approach found in unsound static analysis.

**Our goal is 10x programmer productivity and 10x code assurance compared to
  typical embedded development projects.**
...
code does not contain undefined behaviors, including memory-safety violations.
...
- Size types are machine-independent (with the exception of floating point
  sizes).
...
We do not currently have a key-distribution scheme implemented, so all of the
risks associated with using stale, leaked, or default keys are risks of
SMACCMPilot communication.
